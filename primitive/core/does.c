...
_primitive(pr_does) {
    /* At compile time, does> must capture the location of the code that
       ...
       code after does> will be compiled into the new words defined with the
       word now being defined. The compile-time result should be to compile
       op_do_does
       ...
       where op_does presumes that the parameter address immediately follows
       an exit.
